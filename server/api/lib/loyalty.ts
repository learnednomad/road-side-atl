  ...
  try {
    const newBalance = await db.transaction(async (tx) => {
      const existing = await tx
        ...
        .limit(1);
      if (existing.length > 0) return null; // already awarded for this booking
      const [u] = await tx
      ...
      const updatedBalance = u.balance + points;
      await tx.update(users).set({ loyaltyPoints: updatedBalance }).where(eq(users.id, userId));
      return updatedBalance;
    });
    // Novu: notify customer of points earned (only on a fresh award)
    if (newBalance != null) {
      void triggerNovu(
        WF.loyaltyEarned,
        custSub(userId),
        { points, balance: newBalance },
        { transactionId: `${bookingId}:loyalty` },
      );
    }
  } catch (err) {
